...
public class DESUtils {
    ...
    private final static String TAG = "DESUtils";
...
//        Log.d(TAG, "miWen = " + EncodingUtils.getString(byteMiW, "UTF-8"));
    ...
    public String DecodeStr(String str, int len) {
        ...
        byte[] byteMiW;
        DESUtils m = new DESUtils();
        byteMiW = DESUtils.hexStr2ByteArr(str);
        i2 = m.DECRYPT(DESUtils.strDefaultKey.getBytes(), byteMingW,byteMiW, byteMiW.length);
    ...
    public byte[] Decode(byte[] byteMiWen, int len) {
        ...
        DESUtils m = new DESUtils();
        i2 = m.DECRYPT(DESUtils.strDefaultKey.getBytes(), dataByte,byteMiWen, len);
        byte[] byteMingW = new byte[i2];
        System.arraycopy(dataByte, 0, byteMingW, 0, i2);
//        Log.d(TAG, "mingWen = " + new String(byteMingW, "UTF-8"));
        return byteMingW;
    }
     ...
     */
    public static String DESUtils(String msg) {
        DESUtils m = new DESUtils();
     ...
     * @return byte[] 加密之后的流
     */
    public static byte[] DESUtils(byte[] buf, int len) {
        DESUtils m = new DESUtils();
//        LogUtil.d(TAG, "encryptionMsg = " + EncodingUtils.getString(buf, "UTF-8"));
...
//        LogUtil.d(TAG, "msg = " + msg);
    ...
    public static byte[] DesDecryption(byte[] buf, int len) {
        DESUtils m = new DESUtils();
//        LogUtil.d(TAG, "decryptionMsg = " + EncodingUtils.getString(buf, "UTF-8"));
